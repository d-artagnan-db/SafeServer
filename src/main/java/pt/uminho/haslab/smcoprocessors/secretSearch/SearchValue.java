...
public class SearchValue extends AbstractSearchValue {
    ...
    public List<Boolean> evaluateCondition(List<byte[]> cmpValues,
        ...
        try {
            ...
            LOG.debug("Player with id " + player.getPlayerID() + " is going to run protocol "
                    + condition);
			 ...
			 */
            LOG.debug("Player with id " + player.getPlayerID() + " has " + value.size() + " values and " + cmpValues.size() + " cmpValues");

            if (value.size() == 1 && cmpValues.size() > 1) {
                LOG.debug("Player with id " + player.getPlayerID() + " is replicating values");
            ...
            LOG.debug("Player with id " + player.getPlayerID() + " completed protocol");

            if (player.isTargetPlayer()) {
                LOG.debug("Player with id " +
                        +player.getPlayerID()
                ...
                LOG.debug("Player with id " +
                        +player.getPlayerID() + " is declassifying results");
                ...
                LOG.debug("Player with id " + player.getPlayerID() + " will send protocol results");
                player.sendProtocolResults(targetPlayer,
                        createBatchSearchResults(result, rowID));
                LOG.debug("Player with id " + player.getPlayerID() + " is waiting for filterIndexes");
