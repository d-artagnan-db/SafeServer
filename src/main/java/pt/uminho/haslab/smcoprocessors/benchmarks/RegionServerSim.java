...
public class RegionServerSim extends TestRegionServer {
	...
	private SearchCondition getSearchCondition(Condition cond,int nBits, byte[] secTwo) {
        ...
        for(Condition cond: conds){
            for(int nbit: nbits){
                ...
                while(elapsed < runTime){
                    ...
                    RequestIdentifier ident = new RequestIdentifier(breqID, bregionID);
                    ...
                    SearchCondition scond = getSearchCondition(cond, nbit, valTwo.toByteArray());
                    scond.evaluateCondition(valOne.toByteArray(), breqID, player);
                    reqID +=1;
                    regionID+=1;
